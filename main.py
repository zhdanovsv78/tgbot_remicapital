...
def callback_inline(call):
    ...
    if call.data == "confirm":
        ...
        contact = f'<b>TG-данные</b>: ' + call.message.chat.first_name + ' ' + call.message.chat.last_name + '\n' + '@' + call.message.chat.username
        lid = lid + '\n' + contact
...
def four_q(message):
    ...
    bot.register_next_step_handler(send, confirm)
...
def confirm(message):
    ...
    markup = telebot.types.InlineKeyboardMarkup()
    btn_confirm = types.InlineKeyboardButton('Отправить', callback_data='confirm')
    btn_cancel = types.InlineKeyboardButton('Отмена', callback_data='cancel')
    ...
    now = datetime.datetime.now()
    lid = "<b>Заявка " + now.strftime("%d-%m-%Y %H:%M") + "</b>\n" + lid
    bot.send_message(message.chat.id, lid, reply_markup=markup, parse_mode='html')
